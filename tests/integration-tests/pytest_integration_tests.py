...
def test_count_words(text_files_expected):
    text, expected, not_words = text_files_expected
...
p_expected = {p_word3['word']: [3, '(from 100 to 1000)', '', ''],
              ...
              p_word2['word']: [2, '(l2)', '', '']}
@pytest.fixture(params=[(p_text, p_base, p_files, p_expected, p_not_words)], ids=['text432not'])
def text_files_expected(tmp_SUT_path, request):
    text, base, files, expected, not_words = request.param
    create_txt(tmp_SUT_path, base, files)
    yield text, expected, not_words


def test_get_sorted_list_from_base(files_first_expected):
    base, first, exp = files_first_expected
    if first is None:
...
('l1', (['hello', 'hi'], ['goodbye'], ['bye']), None, None),
...
def files_first_expected(tmp_SUT_path, request):
...
@pytest.fixture(params=[('l1', 'hello')], ids=['l1_h'])
