...
def test_write_read_base(tmp_SUT_path, base, word, utils):
    ...
    assert word == utils.file_content(os.path.join(tmp_SUT_path, base), word)
...
def test_print_base(base_files, input_command, capsys, get_last_lines, expected):
...
ids=['1word', '3words_2del', 'try_wrong_symbol', 'try_without_data', 'try_empty_base'],
                         indirect=['base_files', 'input_command'])
def test_remove_from_base(tmp_SUT_path, base_files, input_command, capsys, get_last_lines, expected_msg, expected_base):
...
# As static method in Util helper class
'''
...
# Moved to conftest.py
# As inner function of the fixture
'''
