...
def count_words(text):
    l1 = [i for i in (j for j, in c.execute("""SELECT word FROM l1"""))]
    l2 = [i for i in (j for j, in c.execute("""SELECT word FROM l2"""))]
    words_counter = {}
    not_words = set()
    for w in text.split():
        w = w.lower()
...
def rem_words(base, str_words):
    ...
    removed = []
    for w in list_words:
        c.execute(f"""SELECT word FROM {base} WHERE word IS "{w}" """)
        if c.fetchall():
            c.execute(f"""DELETE FROM {base} WHERE word IS "{w}" """)
            conn.commit()
            removed.append(w)
        else:
