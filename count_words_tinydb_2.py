"""
Time of checking is reduced by creating json file for each start letter (a.json, b.json, ...)
instead of one, like l1.json.

Comparison:

add 10 000 words to the base with 10 000 words
(two functions for each addition: word_in_base, add_to_base)
count_words_tinydb_.py  : 1075 sec.
count_words_tinydb_2.py : 119 sec.

rem_words (remove 10 000 words from base with 20 000 words)
count_words_tinydb_.py  : 559 sec.
count_words_tinydb_2.py : 47 sec.
"""
...
from words_dicts1 import top_100_english_words, from100_to1000_basic_words
...
for d in ('db', 'db/tiny', 'db/tiny/l1', 'db/tiny/l2'):
    dir_path = os.path.join(SCRIPT_DIR, d)
    if not os.path.isdir(dir_path):
        os.mkdir(dir_path)

# json file for every letter (a..z)
db = {'l1': {}, 'l2': {}}
for i in db:
    for j in string.ascii_lowercase:
        db_file_path = os.path.join(SCRIPT_DIR, 'db', 'tiny', i, j+'.json')
        db[i][j] = TinyDB(db_file_path)
...
def count_words(text):
    l1, l2 = [], []
    for letter in db['l1']:
        letter_list = [i['word'] for i in db[i][letter].all()]
        l1.extend(letter_list)
    for letter in db['l2']:
        letter_list = [i['word'] for i in db[i][letter].all()]
        l2.extend(letter_list)
    words_counter = {}
    not_words = set()
    for w in text.split():
        w = w.lower()
...
def add_to_base(base, word):
    db[base][word[0]].insert({'word': word})
...
def print_base(base, first_letters=None):
    if first_letters:
        ...
        data_from_base = db[base][stripped_fl[0]].search(Query().word.matches(stripped_fl))
    else:
        data_from_base = []
        for letter in db[base]:
            letter_list = db[base][letter].all()
            data_from_base.extend(letter_list)
...
def word_in_base(base, word):
    ...
    l_base = (i['word'] for i in db[base][word[0]].all())
    if word in l_base:
        print(f'{word}, already in ({base})')
        return True
...
def rem_words(base, str_words):
    ...
    removed = []
    for w in list_words:
        if db[base][w[0]].remove(where('word') == w):
            removed.append(w)
        else:
            print(f'{w}, not found in ({base})')
    if removed:
